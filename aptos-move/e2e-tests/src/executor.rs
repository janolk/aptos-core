...
use aptos_types::{
    account_config::{
        fungible_asset_metadata::Supply, new_block_event_key, AccountResource, CoinInfoResource,
...
impl FakeExecutor {
    ...
    pub fn add_lite_account_data(&mut self, lite_account_data: &LiteAccountData) {
        ...
        if new_added_supply != 0 {
            ...
            let mut supply = bcs::from_bytes::<Supply>(
                fa_resource_group.group.get(&Supply::struct_tag()).unwrap(),
            )
            .unwrap();
            supply.current += new_added_supply as u128;
            fa_resource_group
                .group
                .insert(Supply::struct_tag(), bcs::to_bytes(&supply).unwrap())
                .unwrap();
            self.data_store.add_write_set(
                &WriteSetMut::new(vec![(
