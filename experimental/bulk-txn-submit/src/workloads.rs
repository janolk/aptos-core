...
pub fn create_account_addresses_work(destinations_file: &str, only_success: bool) -> Result<Vec<AccountAddress>> {
    read_to_string(Path::new(destinations_file))?
        ...
        .filter(|s| !s.is_empty())
        .map(|text| AccountAddress::from_str_strict(text).map_err(|e| anyhow!("failed to parse {}, {:?}", text, e)))
...
fn parse_line_vec(line: &str) -> Result<(AccountAddress, AccountAddress)> {
    ...
    Ok((
        AccountAddress::from_str_strict(first).map_err(|e| anyhow!("failed to parse {}, {:?}", first, e))?,
        AccountAddress::from_str_strict(second).map_err(|e| anyhow!("failed to parse {}, {:?}", second, e))?,
    ))
}
