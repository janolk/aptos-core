...
use aptos_experimental_bulk_txn_submit::coordinator::{create_sample_addresses, execute_return_worker_funds, execute_submit, CleanAddresses, CreateSampleAddresses, SubmitArgs};
use rand::{seq::SliceRandom, thread_rng};
use std::collections::HashSet;
use aptos_experimental_bulk_txn_submit::workloads::{
    create_account_addresses_work,
...
async fn create_work_and_execute(args: Submit) -> Result<()> {
    ...
    match &args.work_args {
        ...
        WorkTypeSubcommand::ReturnWorkerFunds => {
            execute_return_worker_funds(args.submit_args.transaction_factory_args, args.submit_args.accounts_args, cluster, &coin_source_account).await
        },
    }
}
